...
Use the bundled `scripts/assumption_checks.py` module. Resolve `<SKILL_DIR>` as the directory containing this `SKILL.md`; run Python from `<SKILL_DIR>` or add its `scripts/` directory to `sys.path`:
