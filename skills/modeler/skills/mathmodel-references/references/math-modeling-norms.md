...
| 小节 | 主要使用方 |
| --- | --- |
| [赛题理解与子问题识别](#赛题理解与子问题识别) | `$mathmodel-analysis` |
| [假设与模型建立](#假设与模型建立) | `$mathmodel-analysis` |
...
| [评价类模型详细指南](#评价类模型详细指南) | `$mathmodel-analysis` `$mathmodel-coding` |
| [预测类模型详细指南](#预测类模型详细指南) | `$mathmodel-analysis` `$mathmodel-coding` |
| [优化类模型详细指南](#优化类模型详细指南) | `$mathmodel-analysis` `$mathmodel-coding` |
| [机理/动力学类模型详细指南](#机理动力学类模型详细指南) | `$mathmodel-analysis` `$mathmodel-coding` |
| [图论与网络类模型](#图论与网络类模型) | `$mathmodel-analysis` `$mathmodel-coding` |
| [统计分析与机器学习](#统计分析与机器学习) | `$mathmodel-analysis` `$mathmodel-coding` |
| [论文写作规范补充](#论文写作规范补充) | `$mathmodel-writing` |
| [美赛（MCM/ICM）专项规范](#美赛mcmicm专项规范) | `$mathmodel-writing` |
...
- CUMCM 使用中文摘要和关键词；MCM/ICM 才使用独立的 Summary Sheet。无论采用哪种赛制，摘要或 Summary Sheet 都要覆盖每个实际子问题的方法和关键结果，数值必须与结果记录一致。
- 公式出现后要解释符号含义；图表出现前后要有引导和解释，不能连续堆图。
- 章节结构跟随赛题类型、实际顶层问题数量和论文模板；不得因为模板带有三个问题文件就强行写成三问论文。统计建模可按研究逻辑链组织，不必硬凑“问题一二三”。
...
- 警告包括：未引用备用图、章节过短、图表后缺少解释、caption 过长、外部资料使用但未引用、代码完整复现耗时过长。
...
- 多重共线性检测：较高 VIF 是诊断信号，不是自动删变量规则。结合变量的理论必要性、样本量、预测目标、系数稳定性和验证集表现，选择保留、变量组合、正则化或重新设定模型，并报告处理依据。
...
- CUMCM 中文摘要（含标题和关键词）原则上不超过一页，不需要翻译成英文；电子版论文从摘要页开始，正文不应包含目录。
- 美赛 Summary Sheet 是独立一页，应包含方法亮点、关键结论和模型创新点，不是摘要复制粘贴版。

### 论文结构完整性要点

完整的竞赛论文必须包含：
1. **中文摘要或 Summary**：方法 + 结论 + 数值结果
...
7. **结果检验与稳健性分析**：根据题型选择误差、交叉验证、残差、稳定性、情景或灵敏度分析，说明模型适用边界
8. **模型评价与推广**：优缺点、改进方向、推广场景
9. **参考文献**：只引用真实存在的文献，格式规范

### 灵敏度分析规范

- 结果检验是必要内容，但不强制所有题型使用同一种灵敏度分析；应根据模型类型选择误差、稳定性、情景或参数扰动检验。
