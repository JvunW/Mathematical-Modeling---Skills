...
3. **Develop Search Strategy**:
   ...
   - Use `$paper-lookup` or the host's available web search for initial scoping before formal database searches
...
1. **Multi-Database Search**:
   ...
   Select databases appropriate for the domain. Start with `$paper-lookup` for reproducible scholarly retrieval and supplement it with the host's available web search for discovery. Save exact queries, dates, result limits, counts, and normalized records under the workspace's `sources/` directory.
