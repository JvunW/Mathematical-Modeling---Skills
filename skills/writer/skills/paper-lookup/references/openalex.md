...
- **API key required for every API request** as of February 13, 2026. Free keys are available at https://openalex.org/settings/api
- Pass as: `?api_key=YOUR_KEY`
- The legacy `mailto` polite pool has been retired; `mailto` is ignored
...
**Example:**
```
https://api.openalex.org/works?search=CRISPR+gene+therapy&filter=from_publication_date:2023-01-01&sort=cited_by_count:desc&per_page=10&api_key=YOUR_KEY
```
...
**Example:**
```
https://api.openalex.org/works?filter=from_publication_date:2024-01-01,type:article,is_oa:true,cited_by_count:>50&api_key=YOUR_KEY
```
...
HTTP 403 indicates a forbidden request or invalid API key; HTTP 429 indicates that the daily allowance or rate limit was exceeded. Error responses include a message field. Use bounded exponential backoff for 429 and transient 5xx responses, but do not retry missing or invalid credentials indefinitely.
