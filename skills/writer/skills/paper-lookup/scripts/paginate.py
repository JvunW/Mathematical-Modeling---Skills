...
or Crossref. OpenAlex requires OPENALEX_API_KEY; NCBI_API_KEY and S2_API_KEY
raise limits where relevant.
"""
...
def _openalex_url(query: str, state: Any, limit: int) -> str:
    ...
    params = {"per_page": str(min(limit, 100)), "cursor": str(state)}
    key = os.environ.get("OPENALEX_API_KEY", "").strip()
    if not key:
        raise RuntimeError(
            "OpenAlex requires OPENALEX_API_KEY for every API request; "
...
def main(argv: list[str] | None = None) -> int:
    ...
    api = APIS[args.api]
    if args.api == "openalex" and not os.environ.get("OPENALEX_API_KEY", "").strip():
        fail(
            "OpenAlex requires OPENALEX_API_KEY for every API request; "
            "get a free key at https://openalex.org/settings/api",
            code=2,
