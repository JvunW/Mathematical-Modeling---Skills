...
description: "Verify academic citations and reference relevance. Use to extract references from LaTeX or PDF, check formatting, confirm existence through Crossref, Semantic Scholar, OpenAlex, PubMed, arXiv or dblp, and flag semantic mismatches."
---
...
**If the CLI fails due to network/SSL issues**, use the current harness's available web search to query suspicious citations (especially those marked "unverifiable"), supplementing the verification results.
...
- Cited paper: `title`, `abstract` (from API results or available web search), `venue`
...
2. Obtain the cited paper's abstract via Semantic Scholar or available web search as `cited_text`
