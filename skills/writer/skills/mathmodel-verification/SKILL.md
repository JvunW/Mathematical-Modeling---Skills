...
8. **核对图表与引用。** 所有路径存在、媒体已嵌入、caption 合理、正文有解释；引文对应真实且已核验的条目。由 `$mathmodel-figure-templates` 生成的图还应核对 Figure Contract、可编辑源文件、真实数据来源、矢量输出和 `figure-qa.json`；`pass_with_skips` 中的跳过项必须在最终验收中补做或明确保留风险。
