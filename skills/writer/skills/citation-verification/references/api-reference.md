...
- **文档**: https://developers.openalex.org/
- **限制**: 自 2026 年 2 月 13 日起所有 API 请求都需要免费 API Key；通过 `OPENALEX_API_KEY` 环境变量读取，不得记录或输出密钥。旧的 `mailto` polite pool 已废止

### 常用查询方式

#### 按标题搜索
```
GET https://api.openalex.org/works?search={encoded_title}&per_page=5&api_key={API_KEY}
```

#### 按 DOI 查询
```
GET https://api.openalex.org/works/doi:{doi}?api_key={API_KEY}
```

### 返回字段（常用）
- `results[].display_name` - 论文标题
- `results[].publication_year` - 发表年份
- `results[].primary_location.source.display_name` - 期刊/会议名
- `results[].doi` - DOI
