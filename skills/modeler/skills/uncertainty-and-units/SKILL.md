...
atomically with private permissions, and refuse to overwrite without `--force`. Resolve
...
```text
python "<SKILL_DIR>/scripts/propagate_uncertainty.py" \
...
```text
python "<SKILL_DIR>/scripts/uncertainty_budget.py" --template
python "<SKILL_DIR>/scripts/uncertainty_budget.py" --spec budget.json --format markdown
```

Save the first command's JSON output as `budget.json` using the current harness's file-writing capability before running the second command.
...
```text
python "<SKILL_DIR>/scripts/format_result.py" \
...
```text
python "<SKILL_DIR>/scripts/convert_units.py" \
...
```text
python "<SKILL_DIR>/scripts/audit_units.py" \
...
```text
python "<SKILL_DIR>/scripts/check_plausibility.py" \
