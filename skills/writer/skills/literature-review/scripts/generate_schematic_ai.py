...
def _resolve_api_key(explicit: Optional[str] = None) -> Optional[str]:
    """Resolve the OpenRouter key from an in-process value or the environment."""
    if explicit:
        return explicit
...
"""
    ...
    def __init__(self, api_key: Optional[str] = None, verbose: bool = False):
        ...
        """
        # Prefer an explicit in-process value, then the process environment.
        ...
        if not self.api_key:
            raise ValueError(
                ...
                "  2. Pass api_key programmatically without logging it\n"
...
Environment:
  OPENROUTER_API_KEY    OpenRouter API key (required)
    ...
    # Read credentials from the environment so they do not leak via process listings.
    api_key = _resolve_api_key()
    if not api_key:
        print("Error: OPENROUTER_API_KEY not found")
        print("\nSet OPENROUTER_API_KEY in the environment or a secret manager, then retry.")
        sys.exit(1)
